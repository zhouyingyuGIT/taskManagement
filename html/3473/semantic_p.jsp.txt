...
<body>
  ...
  <script src="./data/material.json"></script>
  ...
  $(function(){
    ...
    Sd.prototype.interval = function(obj) {        
        ...
        $('#con-word').hide();
        $('#con-pic').hide();
        console.log('interval:'+obj.is_correct);
        if(obj.is_correct){
          $('#pause-30 h1').text('正确！能否再快一些！');
        }else{
          $('#pause-30 h1').text('反应错误！');
        }        
        $('#pause-30').show();
    ...
    Sd.prototype.picAssignFun = function() {
      $('#pic_1').attr('src', './img/'+this.data[this.i]["6"]+'.png');
      $('#pic_2').attr('src', './img/'+this.data[this.i]["7"]+'.png');
      $('#pic_3').attr('src', './img/'+this.data[this.i]["8"]+'.png');
    };
