...
function play_sound(data)
{
    ...
    div.innerHTML = '<embed src="'+data+'" loop="0" autostart="true" hidden="true" />';
}
...
;(function () {
    ...
    window.onload = function () {
            ...
            switch (pressKeyCpde) {
                ...
                case 83:
                    ...
                    if (!recoveryTest.isRecoveryTest) {
                       ...
                       // 开始恢复测试
                       if (recoveryTest.isRecoverSoundTest) {
                            recoveryTest.startSoundTest();
                            recoveryTest.isRecoveryTest = false;
                       } else {
            ...
            part1: function(elId, isSoundTest) {
                setTimeout(function () {
                    ...
                    if (BisRecoveryTest) {
                        if (isSoundTest) {
                            // 听
                            play_sound("js/soudTipsPart1.mp3");
                            // window.speechSynthesis.speak(soudTipsPart1);
                        } else {
                            play_sound("js/Part1.mp3");
                    ...
                    hotTestViewTimer = setInterval(function () {
                        ...
                        setTimeout(function () {
                            ...
                            if (hotTestViewCount === 10) {
                                ...
                                if (BisSoundTest) {
                                    !BisRecoveryTest &&  exerciseTest.init();
                                } else {
        ...
        var recoverySoundTestArr = ['11111', '11111'];
        var recoveryTest = {
            isRecoveryTest: false,
            recoveryViewTestResultData: [],
            recoverySoundTestResultData: [],
            index: 1,
            ...
            startViewTest: function() {
                ...
                $().getId('recoveryViewTestPart1').addClass('hide');
                        ...
                        (function(i, j) {
                            setTimeout(function() {
                                ...
                                if (SneedShowData === 'a') {
                                    // 用户的点击反应
                                    document.addEventListener('click', self.clickRecordCb, false);
                                    $().getId('recoveryViewTestAreaSpan').removeClass('hide').html('1');
                                    self.startTestTime = new Date().getTime();
                                    self.OopsCount += 1;
                                } else if (SneedShowData === 'b') {
                                    $().getId('recoveryViewTestAreaSpan').removeClass('hide').html('2');
                                    self.startTestTime = new Date().getTime();
                                    self.OopsCount += 1;
                                } else if (SneedShowData === '1') { 
                                    $().getId('recoveryViewTestAreaSpan').removeClass('hide').html('');
                                    play_sound("js/1.mp3")
                                    // window.speechSynthesis.speak(soudTipsSpeak1);
                                    self.startTestTime = new Date().getTime();
                                    self.OopsCount += 1;
            ...
            startSoundDesc: function() {
                ...
                setTimeout(function(){
                    ...
                    $().getId('recoverysoundhottestPart1').removeClass('hide');
                }, 1000);
                setTimeout(function(){
                    setTimeout(function(){
                        $().getId('recoverysoundhottestPart1H6').removeClass('visbili-hide');
                        play_sound("js/soudTipsPart1.mp3");
                        // window.speechSynthesis.speak(soudTipsPart1);
                    }, 500);
                    setTimeout(function() {
                    ...
                    setTimeout(function() {
                        console.log(4)
                        $().getId('recoverysoundhottestPartTipsPart1').removeClass('visbili-hide');
                        play_sound("js/Part2.mp3");
                        // window.speechSynthesis.speak(viewTestPart2Sound);
                    ...
                    setTimeout(function() {
                        console.log(5)
                        $().getId('recoverysoundhottestPartTipsPart2').removeClass('visbili-hide');
                        play_sound("js/Part3.mp3");
                        // window.speechSynthesis.speak(viewTestPart3Sound);
                    ...
                    setTimeout(function() {
                        console.log(6)
                        $().getId('recoverysoundhottestPartTipsPart3').removeClass('visbili-hide');
                        play_sound("js/Part4.mp3");
                        // window.speechSynthesis.speak(viewTestPart4Sound);
                    ...
                    setTimeout(function() {
                        console.log(7)
                        $().getId('recoverysoundhottestPartTipsPart4').removeClass('visbili-hide');
                        play_sound("js/Part5.mp3");
                        // window.speechSynthesis.speak(viewTestPart5Sound);
                        self.isRecoverSoundTest = true;
                    }, 7000);
                },1000);
            },
            startSoundTest: function() {
                var self = this;
                $().getId('recoverysoundhottestPart1').addClass('hide');
                setTimeout(function() {
                    $().getId('recoverystarHotTestSound').removeClass('hide');
                }, 1000);
                
                self.recoverySoundTestResultData = [];
                var timesInter =  1000;
                self.index = 0;
                self.OopsCount = 0;
                for (var i=0; i < recoverySoundTestArr.length; i++) {
                    for (var j=0; j < recoverySoundTestArr[i].length; j++) {
                        timesInter += 1000;   
                        (function(i, j) {
                            setTimeout(function() {
                                if (!BtestOnoff) return;
                                ...
                                //if (!isContinue) return;
                                // self.ViewintermissionTime();
                                self.recoverySoundTestResultData[self.index] = {
                                    id: self.index,
                                    time: -9999,
                                    count: 0,
                                    status: 0
                                }
                                var SneedShowData = recoverySoundTestArr[i][j];
                                if (SneedShowData === 'a') {
                                    // 用户的点击反应
                                    // document.addEventListener('click', self.clickRecordCb, false);
                                    $().getId('recoveryViewTestAreaSpan').removeClass('hide').html('1');
                            ...
                            ;(recoverySoundTestArr.length * 5) == (i+1) * (j+1) && self.endTest();
                            }, timesInter);
                        })(i, j);
            ...
            ViewintermissionTime: function() { // 显示隐藏元素1 span的 间隔时间
                setTimeout(function() {
                    $().getId('recoveryViewTestAreaSpan').addClass('hide');
                }, 500);
            },
            clickRecordCb: function() {
                isClick = true;
            ...
            endTest: function(){
                $().getId('recoveryTest').addClass('hide');
                // 所有测试内容结束后 延迟 多久 跳转
                setTimeout(function() {
                    completeAllTest();
                }, 2000);
