...
<div id="box">
    ...
    <div id="item2" class="hide item2">
        ...
        <div style="text-align: left;position: relative;">
            <button style="position: absolute;top: 30px;" id="btn" class="btn btn-lg btn-primary">下一题</button>
        </div>
    </div>
</div>
<!--<output></output>-->
...
<script type="text/javascript">
    $(function () {
        ...
        $("#btn").on("click",function () {
            var absNum,indexX,indexY,PAE=0;
            endTime=new Date().getTime();
            time=(endTime-beginTime).toFixed(0);
            estimate=($(".irs-single").text()).replace(/\s/g,"");
            absNum=(Math.abs(estimate-num)/200);
            indexX=String(absNum).indexOf('.') + 1;
            indexY=String(absNum).length - indexX;
            if(indexY>4){
                PAE=absNum.toFixed(4);
            }else {
                PAE=absNum;
            }
            console.log(PAE);
        ...
        function random(lower, upper) {
            var m=Math.floor(Math.random() * (upper - lower)) + lower;
            lgNum=(Math.log(m)/Math.log(10)).toFixed(4);
            return m;
        }
